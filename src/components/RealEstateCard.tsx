...
export function RealEstateCard({ item }: { item: RealEstate }) {
  return (
    <motion.div whileHover={{ y: -4 }} className="glass rounded-2xl overflow-hidden flex flex-col">
      <Link
        to="/real-estate/$code"
        params={{ code: item.property_code }}
        className="relative aspect-[16/9] overflow-hidden bg-secondary block"
      >
        {item.images?.[0] && (
          <img
            src={item.images[0]}
      ...
      <div className="p-5 flex-1 flex flex-col">
        <Link to="/real-estate/$code" params={{ code: item.property_code }}>
          <h3 className="font-display text-xl font-semibold hover:text-gold transition">
            {item.name}
          </h3>
        </Link>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
          <div className="flex items-center gap-1.5 text-muted-foreground">
            <Maximize className="w-3 h-3 text-primary" /> {item.size}
          </div>
          <div className="flex items-center gap-1.5 text-muted-foreground">
            <Droplets className="w-3 h-3 text-primary" /> {item.water_source}
          </div>
        </div>
