...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-navbar-border bg-navbar text-navbar-foreground shadow-nav backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-7xl flex-nowrap items-center justify-between gap-3 px-3 py-2 sm:px-6 min-[1100px]:gap-5 min-[1100px]:py-0">
        <Link
          to="/"
          className="group flex min-w-0 shrink items-center gap-2 min-[1100px]:w-[13.5rem] min-[1100px]:shrink-0"
        >
          <img
            src={logoAsset.url}
        ...
        <nav className="hidden shrink-0 flex-nowrap items-center gap-1 min-[1100px]:flex">
          {NAV.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              activeOptions={{ exact: item.to === "/" }}
              className="whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium text-navbar-muted transition-colors hover:bg-secondary hover:text-navbar-foreground min-[1220px]:px-4"
        ...
        <div className="flex shrink-0 flex-nowrap items-center gap-1 sm:gap-2">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="hidden shrink-0 items-center gap-2 whitespace-nowrap rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground transition hover:opacity-90 min-[1100px]:inline-flex"
          >
          ...
          {user ? (
            <>
              {isAdmin && (
                <Link
                  to="/admin"
                  className="hidden shrink-0 items-center gap-1 whitespace-nowrap rounded-md border border-navbar-border px-3 py-2 text-sm font-semibold text-gold hover:bg-secondary min-[1100px]:inline-flex"
                >
            ...
            <Link
              to="/auth"
              className="hidden shrink-0 whitespace-nowrap rounded-md border border-navbar-border px-3 py-2 text-sm font-semibold text-navbar-foreground hover:bg-secondary min-[1100px]:inline-flex"
            >
              Sign in
            </Link>
          )}
          ...
          <button
            ...
            className="rounded-md border border-navbar-border bg-secondary p-2 text-navbar-foreground hover:bg-accent min-[1100px]:hidden"
            aria-label="Menu"
          >
        ...
        {open && (
          <>
            <motion.div
              ...
              className="fixed inset-0 z-50 bg-background/85 backdrop-blur-sm min-[1100px]:hidden"
            />
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              ...
              className="fixed inset-y-0 right-0 z-50 flex h-dvh max-h-dvh min-h-dvh w-full flex-col overflow-hidden border-l border-navbar-border bg-navbar-panel text-navbar-panel-foreground shadow-2xl sm:w-[24rem] min-[1100px]:hidden"
            >
