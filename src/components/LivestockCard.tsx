...
export function LivestockCard({ item }: { item: Livestock }) {
  return (
    <motion.div whileHover={{ y: -4 }} className="glass rounded-2xl overflow-hidden flex flex-col">
      <Link to="/livestock/$code" params={{ code: item.livestock_code }} className="relative aspect-[4/3] overflow-hidden bg-secondary block">
        {item.images?.[0] && (
      ...
      <div className="p-4 flex-1 flex flex-col">
        <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{item.breed} · {item.category}</div>
        <Link to="/livestock/$code" params={{ code: item.livestock_code }}>
          <h3 className="mt-1 font-display text-lg font-semibold hover:text-gold transition">{item.name}</h3>
        </Link>
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
          <div>Age: <span className="text-foreground">{item.age}</span></div>
          <div>Weight: <span className="text-foreground">{item.weight}</span></div>
          {item.milk_yield && <div className="col-span-2">Milk: <span className="text-foreground">{item.milk_yield}</span></div>}
        </div>
        ...
        <div className="mt-auto pt-4 flex items-end justify-between">
          <div>
            <div className="text-2xl font-bold gold-text">₹{item.price.toLocaleString("en-IN")}</div>
          </div>
          <a
