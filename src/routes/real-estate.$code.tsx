...
function FarmLandDetail() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("real_estate").select("*").eq("property_code", code).maybeSingle();
      ...
      <div className="grid md:grid-cols-3 gap-10">
        <div className="md:col-span-2">
          <div className="flex items-center gap-2">
            {data.verified && <span className="px-2 py-0.5 rounded-full bg-gold text-gold-foreground text-[10px] font-bold uppercase flex items-center gap-1"><BadgeCheck className="w-3 h-3" /> Verified</span>}
            <span className="text-xs uppercase tracking-widest text-muted-foreground">{data.property_code}</span>
          </div>
          <h1 className="mt-2 font-display text-4xl font-bold">{data.name}</h1>
          <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="w-4 h-4" /> {data.location}</div>
          ...
          {data.suitable_for && data.suitable_for.length > 0 && (
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-2">
                {data.suitable_for.map((s) => <span key={s} className="px-3 py-1 rounded-full bg-accent text-sm">{s}</span>)}
              </div>
            </div>
          )}
          {data.amenities && data.amenities.length > 0 && (
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {data.amenities.map((s) => <span key={s} className="px-3 py-1 rounded-full bg-accent text-sm">{s}</span>)}
              </div>
            </div>
          )}
        </div>
        ...
        <div>
          <div className="glass rounded-2xl p-6 sticky top-24">
            <div className="font-display text-3xl font-bold gold-text">₹{(data.price / 100000).toFixed(1)}L</div>
            {data.price_per_acre && <div className="text-sm text-muted-foreground">₹{(data.price_per_acre / 100000).toFixed(2)}L / acre</div>}
            ...
            <a href={propertyInquiry(data.name, data.property_code)} target="_blank" rel="noreferrer"
